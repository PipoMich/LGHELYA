...
export default function NewRelease() {
  return (
    <section className={`${styles.section} reveal`} id="new-release" aria-label="New Release products">
      <div className={styles.inner}>
        <div className={styles.header}>
          <div>
            <h2 className={styles.title}>NEW RELEASE</h2>
            <div className="orange-bar" />
          </div>
          <Link href="/shop" className="btn-outline-sm" id="view-all-products">VIEW ALL</Link>
        </div>

        <div className={styles.grid}>
          {products.map(p => (
            <article key={p.id} className={styles.card} id={`product-${p.id}`}>
              <Link href={`/product/${p.id}`} className={styles.imgLink} aria-label={`View ${p.name} details`}>
              ...
              <div className={styles.info}>
                ...
                <div className={styles.cardActions}>
                  ...
                  <Link href={`/product/${p.id}`} className={`btn-outline-sm ${styles.viewBtn}`} id={`view-product-${p.id}`}>
                    VIEW
                  </Link>
                </div>
              </div>
